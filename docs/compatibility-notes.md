...
Before Phase 1 production logic, run the temporary inspector from `scripts/debug/phase0-runtime-inspector.js` inside the target Foundry world and paste the console output back into this document so the remaining runtime-only fields can be marked as confirmed.
